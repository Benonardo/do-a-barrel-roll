...
public class SyntaxHighlighter {
	...
	public static Text highlightText(String text) {
		...
		if (debugLog) DoAHackedRoll.LOGGER.info("Begun syntax highlighting");
		
		while (context.getCurrent() != (char)0) {
			if (context.getCurrent() == '$') { //variables
				formattedText.append(String.valueOf(context.getCurrent()));
				context.position++;
				if (debugLog) DoAHackedRoll.LOGGER.info("Begun coloring variable");
				...
				while (isLetter(context.getCurrent()) || context.getCurrent() == '_') {
					...
					context.position++;
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring variable");
				}
				...
				if (Character.isDigit(context.peek()) && context.lastIsNotValue()) {
					...
					context.position++;
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring number");
					...
					context.position++;
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring function");
				} else {
					formattedText.append(formatText(context.getCurrent(), SyntaxType.Operator));
					context.position++;
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring operator");
				}
				...
				while (isLetter(context.getCurrent()) || context.getCurrent() == '_') {
					...
					context.position++;
					if (debugLog) DoAHackedRoll.LOGGER.info("Reading possible function or constant");
				}
				...
				if (isKeyword(builtResult) && context.getCurrent() == '(') {
					...
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring constant");
				} else {
					formattedText.append(formatText(builtResult, SyntaxType.Error));
					if (debugLog) DoAHackedRoll.LOGGER.info("Coloring error");
				}
				...
				context.position++;
				if (debugLog) DoAHackedRoll.LOGGER.info("Skipping parentheses");
			} else if (Character.isWhitespace(context.getCurrent())) { //whitespace
				formattedText.append(String.valueOf(context.getCurrent()));
				context.position++;
				if (debugLog) DoAHackedRoll.LOGGER.info("Skipping whitespace");
			} else { //errors
				formattedText.append(formatText(context.getCurrent(), SyntaxType.Error));
				context.position++;
				if (debugLog) DoAHackedRoll.LOGGER.info("Coloring errors");
			}
		}
		
		if (debugLog) DoAHackedRoll.LOGGER.info("Finished syntax coloring");
